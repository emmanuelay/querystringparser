	...
	case Strings:
		{
			...
			// Wrap Should values in a required disjunction group so Bleve
			// treats it as "must match any": +(field:val1 field:val2)
			if p.OutputCondition == Should && len(p.StringsValue) > 0 {
